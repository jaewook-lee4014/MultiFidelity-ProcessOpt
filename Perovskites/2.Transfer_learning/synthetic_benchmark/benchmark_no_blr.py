#!/usr/bin/env python
"""
Multi-Fidelity Benchmark Comparison (NO BLR for DNN models)

Acquisition Strategy:
- MFGP: EI (Expected Improvement) with GP uncertainty
- All DNN models: argmin(mean) - NO uncertainty estimation, NO BLR

Models:
1. MFGP - BoTorch SingleTaskMultiFidelityGP (EI selection) ← 유일하게 불확실성 추정
2. Sequential - argmin(mean)
3. Progressive - argmin(mean)
4. Curriculum - argmin(mean)
5. Two-Stage Joint - argmin(mean)
6. DNGO-Joint - argmin(mean)
7. DNGO-Gradient - argmin(mean)
8. Knowledge Distillation - argmin(mean)
9. Domain Adaptation (MMD) - argmin(mean)
10. Soft Parameter Sharing - argmin(mean)
11. Pseudo-Labeling - argmin(mean)
12. Adapter - argmin(mean)
...
class ChemistryBenchmark:
    """Chemistry benchmark from CSV files (COFs, FreeSolv, Polarizability)"""
    ...
    def __init__(self, name, csv_path, cost_ratio, use_smiles=False, minimize=True):
        ...
        Args:
            ...
            cost_ratio: LF cost / HF cost
            use_smiles: If True, uses SMILES fingerprints; if False, uses all feature columns
            minimize: Whether to minimize (True) or maximize (False) the target
        """
        self.name = name
        self.cost_ratio = cost_ratio
        ...
        if use_smiles:
            # Use SMILES fingerprints (FreeSolv, Polarizability)
            # Find the SMILES column (usually first column)
            ...
            # Use all columns except HF and LF (COFs)
        ...
        # For minimization, f_star is the minimum HF value
        if minimize:
            self.f_star = self.y_hf.min()
        else:
            self.f_star = self.y_hf.max()
        ...
        print(f"Created {name}: {self.n_candidates} samples, dim={self.dim}, R²={self.r2:.3f}, ρ={cost_ratio}")
    ...
    def _smiles_to_features(self, smiles_list):
        ...
        for smi in smiles_list:
            ...
            feat = [
                len(smi),                           # SMILES length
                smi.count('C'),                     # Carbon count
                smi.count('N'),                     # Nitrogen count
                smi.count('O'),                     # Oxygen count
                smi.count('F'),                     # Fluorine count
                smi.count('Cl'),                    # Chlorine count
                smi.count('Br'),                    # Bromine count
                smi.count('S'),                     # Sulfur count
                smi.count('('),                     # Branch count
                smi.count('='),                     # Double bond count
                smi.count('#'),                     # Triple bond count
                smi.count('['),                     # Ring atom count
                smi.count('c'),                     # Aromatic carbon
                smi.count('n'),                     # Aromatic nitrogen
                smi.count('1') + smi.count('2'),    # Ring indicators
            ]
...
def run_all_benchmarks(n_seeds=3, base_seed=42):
    ...
    benchmarks = {
        ...
        'Park-4D': SyntheticBenchmark(
            'Park-4D', park_hf, park_lf, dim=4,
            alpha=0.5, cost_ratio=0.2, f_star=0.0, grid_size=10
        ),
        # Chemistry benchmarks
        'COFs': ChemistryBenchmark(
            'COFs', data_dir / 'cofs.csv', cost_ratio=0.1,
            use_smiles=False, minimize=True  # Use 14 molecular features
        ),
        'FreeSolv': ChemistryBenchmark(
            'FreeSolv', data_dir / 'freesolv.csv', cost_ratio=0.1,
            use_smiles=True, minimize=True  # Lower solvation energy is better
        ),
        'Polarizability': ChemistryBenchmark(
            'Polarizability', data_dir / 'polarizability.csv', cost_ratio=0.1,
            use_smiles=True, minimize=True  # Lower polarizability is target
        ),
    }

    # Budget per benchmark (based on dataset size and cost ratio)
    budgets = {
        'Branin-Fav': 50,
        'Branin-Unfav': 50,
        'Park-4D': 50,
...
def main():
    ...
    output_dir = Path(f'benchmark_no_blr_{timestamp}')
    output_dir.mkdir(exist_ok=True)

    print("=" * 80)
    print("Multi-Fidelity Benchmark (NO BLR)")
    print("MFGP: EI | Others: argmin(mean)")
    print("=" * 80)
