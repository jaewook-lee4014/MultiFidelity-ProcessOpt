#!/usr/bin/env python
"""
Plot regret convergence - shift MF curves by dividing by start point ratio
Log scale에서 평행이동 = 나누기 (비율 적용)
"""
...
def main():
    ...
    # Plot MF models with ratio shift (log scale 평행이동)
    for scenario in ['favorable', 'unfavorable']:
        ...
        for model in ['GP_MFGP', 'DNGO_Joint']:
            if model in scenario_results:
                ...
                # Calculate ratio and divide (log scale 평행이동)
                ratio = mf_start / gp_start
                regrets_shifted = regrets_interp / ratio

                print(f"{model} ({scenario}): start={mf_start:.4f}, ratio={ratio:.4f}, new_start={regrets_shifted[0]:.4f}")
    ...
    output_path = Path('viz_mf_bo_steps_20251219_131928/sf_mf_regret_adjusted_v5.png')
