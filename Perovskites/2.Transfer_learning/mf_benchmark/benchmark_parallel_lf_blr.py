...
def run_bo_lf_blr(benchmark, model_class, budget, seed=42, device=None, sampling_method='fps'):
    """
...
def run_combination(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        try:
            # Use LF-BLR BO loop
            result = run_bo_lf_blr(benchmark, model_class, budget, seed, device, sampling_method)
...
def main():
    ...
    output_dir = Path(f'benchmark_lf_blr_{timestamp}')
    ...
    print("  - MFGP: Uses its own GP-based UQ (unchanged)")
    ...
    print(f"\nCombinations: {total_combinations} (7 benchmarks × 12 models)")
