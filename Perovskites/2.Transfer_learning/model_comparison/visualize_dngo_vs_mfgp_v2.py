#!/usr/bin/env python
"""
Compare DNGO+BO vs MFGP with uncertainty visualization for BOTH models
"""
import sys
import numpy as np
import torch
...
def main():
    ...
    # 2. Train DNGO+BO with BLR for uncertainty
    print("\nTraining DNGO+BO...")
    set_seeds(SEED)
    t0 = time.time()
    ...
    # Extract features and fit BLR for uncertainty estimation
    ...
    # Predict with uncertainty using BLR
    features_test = dngo.extract_features(data['X_test'])
    dngo_pred, dngo_var = blr.predict_batch(features_test)
    dngo_std = np.sqrt(dngo_var)
    ...
    print(f"DNGO mean uncertainty (σ): {dngo_std.mean():.4f}")
    print(f"MFGP mean uncertainty (σ): {mfgp_std.mean():.4f}")
    ...
    dngo_std_sorted = dngo_std[sort_idx]
    x_axis = np.arange(len(y_true_sorted))

    # ========== PLOT 1: Stacked comparison with uncertainty ==========
    ...
    ax.fill_between(x_axis,
                    ...
                    mfgp_pred_sorted + 2*mfgp_std_sorted,
                    alpha=0.3, color='blue', label=f'±2σ (mean σ={mfgp_std.mean():.3f})', zorder=1)
    ...
    # DNGO+BO (bottom) with uncertainty
    ax = axes[1]
    ax.scatter(x_axis, y_true_sorted, c='black', s=10, alpha=0.7, label='True HF', zorder=3)
    ...
    ax.fill_between(x_axis,
                    ...
                    dngo_pred_sorted + 2*dngo_std_sorted,
                    alpha=0.3, color='green', label=f'±2σ (mean σ={dngo_std.mean():.3f})', zorder=1)
    ...
    plt.tight_layout()
    plt.savefig('dngo_vs_mfgp_with_uncertainty.png', dpi=150, bbox_inches='tight')
    print("\nSaved: dngo_vs_mfgp_with_uncertainty.png")
    plt.close()

    # ========== PLOT 2: Parity plots with uncertainty ==========
    ...
    ax.set_title(f'MFGP\nRMSE={mfgp_rmse:.3f}, R²={mfgp_r2:.3f}, σ={mfgp_std.mean():.3f}', fontsize=14)
    ax.set_xlim(mn, mx); ax.set_ylim(mn, mx)
    ax.set_aspect('equal')
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)

    # DNGO+BO parity with uncertainty
    ax = axes[1]
    ax.plot([mn, mx], [mn, mx], 'k--', lw=1.5, alpha=0.5, label='y=x')
    ax.errorbar(data['y_test'], dngo_pred, yerr=2*dngo_std, fmt='o', ms=5,
                alpha=0.5, elinewidth=0.5, capsize=0, color='green', label='DNGO+BO ±2σ')
    ax.set_xlabel('True Bandgap (eV)', fontsize=12)
    ax.set_ylabel('Predicted Bandgap (eV)', fontsize=12)
    ax.set_title(f'DNGO+BO\nRMSE={dngo_rmse:.3f}, R²={dngo_r2:.3f}, σ={dngo_std.mean():.3f}', fontsize=14)
    ax.set_xlim(mn, mx); ax.set_ylim(mn, mx)
    ax.set_aspect('equal')
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('dngo_vs_mfgp_parity_uncertainty.png', dpi=150, bbox_inches='tight')
    print("Saved: dngo_vs_mfgp_parity_uncertainty.png")
    plt.close()

    # ========== PLOT 3: Uncertainty distribution comparison ==========
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))

    ax = axes[0]
    ax.hist(mfgp_std, bins=30, alpha=0.7, color='blue', edgecolor='black', label='MFGP')
    ax.axvline(mfgp_std.mean(), color='red', linestyle='--', lw=2, label=f'Mean={mfgp_std.mean():.3f}')
    ...
    ax = axes[1]
    ax.hist(dngo_std, bins=30, alpha=0.7, color='green', edgecolor='black', label='DNGO+BO')
    ax.axvline(dngo_std.mean(), color='red', linestyle='--', lw=2, label=f'Mean={dngo_std.mean():.3f}')
    ax.set_xlabel('Prediction Std (σ)', fontsize=12)
    ax.set_ylabel('Count', fontsize=12)
    ax.set_title('DNGO+BO Uncertainty Distribution', fontsize=14)
    ax.legend()
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig('dngo_vs_mfgp_uncertainty_dist.png', dpi=150, bbox_inches='tight')
    print("Saved: dngo_vs_mfgp_uncertainty_dist.png")
    plt.close()

    print("\n" + "="*60)
    print("SUMMARY")
    print("="*60)
    print(f"{'Model':<12} {'RMSE':>8} {'R²':>8} {'ρ':>8} {'Mean σ':>10} {'Time':>8}")
    print("-"*60)
    print(f"{'MFGP':<12} {mfgp_rmse:>8.4f} {mfgp_r2:>8.4f} {mfgp_sp:>8.4f} {mfgp_std.mean():>10.4f} {mfgp_time:>7.1f}s")
    print(f"{'DNGO+BO':<12} {dngo_rmse:>8.4f} {dngo_r2:>8.4f} {dngo_sp:>8.4f} {dngo_std.mean():>10.4f} {dngo_time:>7.1f}s")
    print("="*60)
