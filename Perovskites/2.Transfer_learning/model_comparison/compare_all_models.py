#!/usr/bin/env python
"""
Comprehensive Model Comparison: MFGP vs DNGO vs BNN

Compares:
1. MFGP (Multi-Fidelity GP using BoTorch)
2. Standard GP (HF only)
3. DNGO + BO
4. DNGO + BO + Progressive Unfreezing
5. BNN + BO

With fixed seeds for reproducibility and visualization of predictions.
"""
import sys
import os
import numpy as np
...
# =============================================================================
# 10 fixed seeds for 10 runs - ensures same data splits across experiments
...
def run_dngo(dataset, X_test, device, bo_trials=20, use_progressive=False):
    """Run DNGO with optional progressive unfreezing."""
    input_dim = dataset['X_low'].shape[1]
    ...
    model.finetune(dataset['X_high'], dataset['y_high'],
                   bo_trials=bo_trials, verbose=False,
                   use_freeze=False, use_progressive_unfreezing=use_progressive)
    y_pred = model.predict(X_test)
    elapsed = time.time() - start

    # DNGO doesn't provide uncertainty by default, use None
    return y_pred, None, elapsed


def run_bnn(dataset, X_test, device, bo_trials=20):
    """Run BNN."""
    input_dim = dataset['X_low'].shape[1]
...
def plot_predictions(y_true, predictions_dict, run_idx, save_dir):
    """
    Plot predictions vs true values for all models.

    Args:
        y_true: True HF values
        predictions_dict: Dict of {model_name: (y_pred, y_std)}
    ...
    for idx, (model_name, (y_pred, y_std)) in enumerate(predictions_dict.items()):
        ...
        # Plot true values
        ax.scatter(x_axis, y_true_sorted, c='black', s=5, alpha=0.5, label='True HF')

        # Plot predictions
        ax.scatter(x_axis, y_pred_sorted, c='blue', s=5, alpha=0.5, label='Predicted')
...
def plot_parity(y_true, predictions_dict, run_idx, save_dir):
    """
    Plot parity plots (predicted vs true) for all models.
    """
    n_models = len(predictions_dict)
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    axes = axes.flatten()
    ...
    for idx, (model_name, (y_pred, y_std)) in enumerate(predictions_dict.items()):
        ...
        ax.plot([min_val, max_val], [min_val, max_val], 'k--', lw=1, label='y=x')

        # Plot predictions with uncertainty
        if y_std is not None:
            ax.errorbar(y_true, y_pred, yerr=2*y_std, fmt='o', markersize=3,
                       alpha=0.5, elinewidth=0.5, capsize=0, label='Predictions')
        else:
            ax.scatter(y_true, y_pred, s=10, alpha=0.5, label='Predictions')
...
def run_comparison(n_runs=10, n_lofi=72, n_hifi=9, bo_trials=20, save_dir='results_mfgp'):
    ...
    # Results storage
    models = ['MFGP', 'Standard_GP', 'DNGO+BO', 'DNGO+BO+Prog', 'BNN+BO']
    ...
    for run in range(n_runs):
        ...
        print("\n3. DNGO + BO...")
        try:
            set_all_seeds(seed)  # Reset seed for reproducibility
            y_pred, y_std, elapsed = run_dngo(dataset, X_test, device, bo_trials=bo_trials, use_progressive=False)
            metrics = calculate_metrics(y_test_hifi, y_pred)
            results['DNGO+BO']['rmse'].append(metrics['rmse'])
            results['DNGO+BO']['r2'].append(metrics['r2'])
            results['DNGO+BO']['spearman'].append(metrics['spearman'])
            results['DNGO+BO']['time'].append(elapsed)
            predictions['DNGO+BO'] = (y_pred, y_std)
            print(f"   RMSE: {metrics['rmse']:.4f}, R²: {metrics['r2']:.4f}, ρ: {metrics['spearman']:.4f}, Time: {elapsed:.1f}s")
        except Exception as e:
            print(f"   DNGO+BO failed: {e}")
            results['DNGO+BO']['rmse'].append(np.nan)
            results['DNGO+BO']['r2'].append(np.nan)
            results['DNGO+BO']['spearman'].append(np.nan)
            results['DNGO+BO']['time'].append(np.nan)

        # 4. DNGO + BO + Progressive Unfreezing
        print("\n4. DNGO + BO + Progressive...")
        try:
            set_all_seeds(seed)
            y_pred, y_std, elapsed = run_dngo(dataset, X_test, device, bo_trials=bo_trials, use_progressive=True)
            metrics = calculate_metrics(y_test_hifi, y_pred)
            results['DNGO+BO+Prog']['rmse'].append(metrics['rmse'])
            results['DNGO+BO+Prog']['r2'].append(metrics['r2'])
            results['DNGO+BO+Prog']['spearman'].append(metrics['spearman'])
            results['DNGO+BO+Prog']['time'].append(elapsed)
            predictions['DNGO+BO+Prog'] = (y_pred, y_std)
            print(f"   RMSE: {metrics['rmse']:.4f}, R²: {metrics['r2']:.4f}, ρ: {metrics['spearman']:.4f}, Time: {elapsed:.1f}s")
        except Exception as e:
            print(f"   DNGO+BO+Prog failed: {e}")
            results['DNGO+BO+Prog']['rmse'].append(np.nan)
            results['DNGO+BO+Prog']['r2'].append(np.nan)
            results['DNGO+BO+Prog']['spearman'].append(np.nan)
            results['DNGO+BO+Prog']['time'].append(np.nan)

        # 5. BNN + BO
        print("\n5. BNN + BO...")
        try:
            set_all_seeds(seed)
            y_pred, y_std, elapsed = run_bnn(dataset, X_test, device, bo_trials=bo_trials)
            metrics = calculate_metrics(y_test_hifi, y_pred)
            results['BNN+BO']['rmse'].append(metrics['rmse'])
            results['BNN+BO']['r2'].append(metrics['r2'])
            results['BNN+BO']['spearman'].append(metrics['spearman'])
            results['BNN+BO']['time'].append(elapsed)
            predictions['BNN+BO'] = (y_pred, y_std)
            print(f"   RMSE: {metrics['rmse']:.4f}, R²: {metrics['r2']:.4f}, ρ: {metrics['spearman']:.4f}, Time: {elapsed:.1f}s")
        except Exception as e:
            print(f"   BNN+BO failed: {e}")
            results['BNN+BO']['rmse'].append(np.nan)
            results['BNN+BO']['r2'].append(np.nan)
            results['BNN+BO']['spearman'].append(np.nan)
            results['BNN+BO']['time'].append(np.nan)

        # Generate plots for this run
        if predictions:
            plot_predictions(y_test_hifi, predictions, run, save_path)
            plot_parity(y_test_hifi, predictions, run, save_path)
    ...
    print("SUMMARY")
    print("=" * 80)
    print(f"{'Model':<20} {'RMSE':>15} {'R²':>15} {'Spearman':>15} {'Time':>12}")
    print("-" * 80)
    ...
    for model in models:
        ...
        print(f"{model:<20} {rmse_str:>15} {r2_str:>15} {sp_str:>15} {time_str:>12}")
    ...
    df.to_csv(save_path / 'summary.csv', index=False)
    ...
    detailed_df.to_csv(save_path / 'detailed_results.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save-dir', type=str, default='results_mfgp')
